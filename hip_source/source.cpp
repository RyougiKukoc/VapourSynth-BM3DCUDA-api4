...
static inline void Aggregation(
    float * VS_RESTRICT dstp, ptrdiff_t dst_stride,
    const float * VS_RESTRICT srcp, ptrdiff_t src_stride,
...
static const VSFrame *VS_CC BM3DGetFrame(
    ...
    if (activationReason == arInitial) {
        ...
        if (d->chroma) {
            ...
            ptrdiff_t s_pitch = vsapi->getStride(src, 0);
            ptrdiff_t s_stride = s_pitch / sizeof(float);
            ...
            for (int plane = 0; plane < d->vi.format.numPlanes; plane++) {
                ...
                ptrdiff_t s_pitch = vsapi->getStride(src, plane);
                ptrdiff_t s_stride = s_pitch / sizeof(float);
                int width_bytes = width * sizeof(float);
...
static void VS_CC BM3DCreate(
    ...
    d->out_vi = d->vi;
    if (d->radius) {
        d->out_vi.height *= 2 * (2 * d->radius + 1);
    }
    ...
    int num_deps = d->ref_node ? 2 : 1;
    BM3DData * data = d.get();
    vsapi->createVideoFilter(
        out, "BM3D", &data->out_vi,
    ...
    if (activationReason == arInitial) {
        ...
        for (int plane = 0; plane < d->src_vi.format.numPlanes; ++plane) {
            if (d->process[plane]) {
                ...
                ptrdiff_t plane_stride = vsapi->getStride(src_frame, plane) / sizeof(float);
...
static void VS_CC VAggregateCreate(
    ...
    VAggregateData * data = d.get();
    vsapi->createVideoFilter(
        out, "VAggregate", &data->src_vi,
        VAggregateGetFrame, VAggregateFree,
        fmParallel, deps, 2, data, core);
    if (!vsapi->mapGetError(out))
        d.release();
}
